...
class DSEBlock(nn.Module):
    ...
    #     print(f"{y_token.shape=}")
    #     diff = self.diffattn(y_token) #* y_token

    #     print(f"{diff.shape=}")
    #     B, H, W = diff.shape
    #     pad_h = (self.input_size - H % self.input_size) % self.input_size
    #     pad_w = (self.input_size - W % self.input_size) % self.input_size

    #     print(f"{pad_w=}, {pad_h=}")

    #     # pad width and height dynamically (zero padding or 'reflect' if you prefer)
    #     diff = F.pad(diff, (0, pad_w, 0, pad_h))
    #     Hp, Wp = diff.shape[1], diff.shape[2]


    #     diff = diff.view(diff.shape[0], diff.shape[2], diff.shape[1]//self.input_size, diff.shape[1]//self.input_size)
    #     diff = diff[:, :H, :W].contiguous()  # crop back to original size
    ...
    def apply_diffattn(self, x):
        ...
        y_token = x.view(B, C, -1).permute(0, 2, 1)  # (B, N, C)
        diff = self.diffattn(y_token)  # (B, N, C) or similar
        # diff = checkpoint(self.diffattn, y_token)

        # Restore spatial structure
        num_tokens = diff.shape[1]
        side = int(num_tokens ** 0.5)  # assume roughly square
        if side * side != num_tokens:
            # if not square, infer height and width from input size ratio
            h_tokens = round(H / self.input_size)
            w_tokens = round(W / self.input_size)
        else:
            h_tokens = w_tokens = side

        diff = diff.permute(0, 2, 1).contiguous().view(B, C, h_tokens, w_tokens)

        # Resize diff to match x spatial dims (interpolation)
